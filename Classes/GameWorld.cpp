...
void GameWorld::CreateHUD()
{
	...
	char buf[64] = {0};
	sprintf(buf, "%d", cash_);
	cash_label_ = CCLabelBMFont::create(buf, "wal_font.fnt");
	cash_label_->setPosition(ccp(190, 755));
	cash_label_->setScale(0.8f);
	addChild(cash_label_, E_LAYER_HUD);

	// create the waves label
	sprintf(buf, "1/%d", num_waves_);
	waves_label_ = CCLabelBMFont::create(buf, "wal_font.fnt");
	waves_label_->setPosition(ccp(500, 755));
	waves_label_->setScale(0.8f);
	addChild(waves_label_, E_LAYER_HUD);

	CCLabelBMFont* label = CCLabelBMFont::create("Waves", "wal_font.fnt");
...
void GameWorld::ReduceLives(int amount)
{
	...
	pumpkin_->runAction(CCSequence::create(CCRotateBy::create(0.1f, -10.0f), CCRotateBy::create(0.1f, 10.0f), CCRotateBy::create(0.1f, -10.0f), CCRotateBy::create(0.1f, 10.0f), NULL));
}
